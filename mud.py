...
class MudCLI:
        ...
        # Handling general commands
            ...
            if utils.settings.config['mud'].getboolean('run_async'):
                if utils.settings.config['mud'].getboolean('run_table'):
                    asyncio.run(self.cmd_runner._run_async_table_view(self.repos.keys(), command_str))
                else:
    ...
    async def _fetch_repo_async(self, repo: str) -> None:
        await asyncio.to_thread(subprocess.run, ['git', 'fetch'], cwd=repo, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
